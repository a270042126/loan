...
export default (bus, store, moduleName, keyName) => {
  ...
  const refresh = (toRoute, fromRoute) => {
    ...
    if (store) {
      store.commit('navigation/REFRESH', { to, from })
    }
  ...
  const record = (toRoute, fromRoute, replaceFlag) => {
    ...
    if (replaceFlag) {
      ...
      if (fromRoute.meta.hasOwnProperty('isKeepAlive') && fromRoute.meta.isKeepAlive && toRoute.meta.hasOwnProperty('isKeepAlive') && toRoute.meta.isKeepAlive) {
        removeRoute(name)
      }
      let toIndex = Routes.lastIndexOf(name)
      if (toIndex === -1) {
